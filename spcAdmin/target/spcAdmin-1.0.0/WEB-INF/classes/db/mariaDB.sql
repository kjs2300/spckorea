...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `CREATE_TABLE`(IN _tableName VARCHAR(200), IN _tableNameTag VARCHAR(200) ,OUT RESULT INT)
BEGIN
	/* 가격을 변경할 변수를 선언한다. */
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `CREATE_TABLE1`(IN _tableName VARCHAR(200), IN _tableNameTag VARCHAR(200), OUT RESULT INT)
BEGIN
	/* 가격을 변경할 변수를 선언한다. */
	DECLARE _SELLPRICE DOUBLE;
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `CREATE_TABLE2`(IN _tableName VARCHAR(200), IN _tableNameTag VARCHAR(200))
BEGIN
	/* 가격을 변경할 변수를 선언한다. */
	DECLARE _SELLPRICE DOUBLE;
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_BOOK`(IN _BOOKID CHAR(5), IN _BOOKNAME VARCHAR(20), _PRICE DOUBLE, _BOOKTYPE VARCHAR(10), OUT RESULT INT)
BEGIN
	/* 가격을 변경할 변수를 선언한다. */
	DECLARE _SELLPRICE DOUBLE;
...
CREATE TABLE IF NOT EXISTS `tb_board_file` (
  `FILE_UUID` varchar(300) NOT NULL COMMENT '파일UUID',
  `FILE_NAME` varchar(300) NOT NULL COMMENT '파일이름',
  `FILE_FULL_PATH` varchar(500) NOT NULL COMMENT '파일전체경로',
  `FILE_SEQ` int(11) NOT NULL AUTO_INCREMENT COMMENT '파일순번',
  `FILE_SIZE` float DEFAULT NULL COMMENT '파일사이즈',
  PRIMARY KEY (`FILE_FULL_PATH`),
  KEY `FILE_SEQ` (`FILE_SEQ`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8;

-- 테이블 데이터 testdb.tb_board_file:~4 rows (대략적) 내보내기
...
INSERT INTO `tb_board_file` (`FILE_UUID`, `FILE_NAME`, `FILE_FULL_PATH`, `FILE_SEQ`, `FILE_SIZE`) VALUES
	('171bb9a4a82c4e38875b686240ce9a2b', '평생교육론 3주_1.pdf', 'c://livrio//upload\\171bb9a4a82c4e38875b686240ce9a2b.pdf', 1, NULL),
	('75498ee9082d49febcd8a2664327635a', '노인복지론_1주-7주.pdf', 'c://livrio//upload\\75498ee9082d49febcd8a2664327635a.pdf', 4, 10975500),
	('c68133b2e9ed40e3b9560965243d05a3', 'tb_board.sql', 'c://livrio//upload\\c68133b2e9ed40e3b9560965243d05a3.sql', 5, 891),
	('e4bb0be45c2b49029655762e9473b5f0', 'mysql tip1.sql', 'c://livrio//upload\\e4bb0be45c2b49029655762e9473b5f0.sql', 2, 1248);
...
CREATE TABLE IF NOT EXISTS `tb_vendor` (
  ...
  `comment` text DEFAULT NULL COMMENT '기타사항',
...
-- 테이블 데이터 testdb.tb_vendor:~2 rows (대략적) 내보내기
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_BOOK`(IN _NOVEL_ADD_PRICE DOUBLE, IN _ART_ADD_PRICE DOUBLE, IN _ETC_ADD_PRICE DOUBLE, OUT RESULT INT)
BEGIN
	/* 종료 구분 변수 */
