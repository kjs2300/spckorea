...
<script type="text/javascript">
	...
	function goBoardUpdate(){
		var frm = document.boardForm;
		frm.action = "<c:url value='/board/boardUpdate.do'/>";
		frm.submit();
	}
	
	/** 게시판 - 파일다운로드 페이지 이동 */
	function goFileDownload(){
		var frm = document.boardForm;
		frm.action = "<c:url value='/board/fileDownload.do'/>";
		frm.submit();
	}
	...
	function goBoardReply(){
		...
		var frm = document.boardForm;
		frm.action = "<c:url value='/board/boardReply.do'/>";
		frm.submit();
	}
	...
	function getBoardDetailCallback(obj){
		...
		if(obj != null){								
			...
			var fileName 		= isEmpty(obj.file_name);
			...
			str += "<tr>";
			str += "<th>내용</th>";
			str += "<td colspan='3'><pre>"+ boardContent +"</pre></td>";
			str += "</tr>";
			
			if (fileName != 'N' )  {
				str += "<th>파일명</th>";
				str += "<td> <a onclick='javascript:goFileDownload();'>"+ fileName +"</a></td>";
				str += "</tr>";		
			}
	...
	<div id="container">
		<div class="inner">	
			...
			<div class="btn_right mt15">
				...
				<button type="button" class="btn black mr5" onclick="javascript:goBoardUpdate();">수정하기1</button>
